...
**你要做的只有两步**（桥不用你启动，交给 AI）：

1. 插件图标右键 →「选项」→ 勾选开启 Agent 模式 → 保存（口令已自动生成，复制它）
2. 把下面这段话发给你的 AI 助手，口令替换成你复制的：

   > 我装了 mp-helper 插件（https://github.com/TuYv/mp-helper ）并开启了 Agent 模式，口令是 xxxx。以后我让你发公众号草稿或查数据时，你负责：clone 或更新这个仓库，用 `node bridge/server.mjs --token 口令` 把桥跑起来（Node >= 18，无依赖，只监听 127.0.0.1），然后按它 README 里的三个 /api 接口操作。

之后 AI 助手可用的接口（都带 `x-bridge-token` 请求头）：
